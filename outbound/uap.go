...
var _ adapter.Outbound = (*UAP)(nil)
...
func NewUAP(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.UAPOutboundOptions) (*UAP, error) {
	...
	outbound.client, err = uap.NewClient(options.UUID, options.Flow, logger)
	if err != nil {
		return nil, err
	}
	outbound.multiplexDialer, err = mux.NewClientWithOptions((*uapDialer)(outbound), logger, common.PtrValueOrDefault(options.Multiplex))
	if err != nil {
		return nil, err
	}
	return outbound, nil
...
func (h *UAP) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
...
func (h *UAP) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	if h.multiplexDialer == nil {
		h.logger.InfoContext(ctx, "outbound packet connection to ", destination)
